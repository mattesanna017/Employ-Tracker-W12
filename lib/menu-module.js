...
const viewOurRoles =() =>{
    ...
    db.query('SELECT employee_role.title, employee_role.salary, department.department_name FROM employee_role INNER JOIN department on department.id = employee_role.department_id', function (err, results) {
        console.log("\n")
        console.table(results);
    });
}
...
const addDepartment = (response) =>{
    db.query(`INSERT INTO department (department_name) VALUES (?)`, response, function (err, results) {
        viewAllDepartments();
    });
}
